...
class Admin_MateriSilabusController extends Zend_Controller_Action
{
	...
	public function indexAction()  {	
		...
		// $id = $this->getRequest()->getParam('id_silabus');
		// $this->view->rows = $this->SilabusService->getDataByIdSilabus($id);
		// $this->view->rows2 = $this->SilabusService->getAllData();
		// var_dump($id_silabus);
		// die();
		// $this->view->rows = $this->MateriSilabusService->getDataByIdSilabus($id_silabus);
	...
	// 	$id_silabus = $this->_getParam('id_silabus', null);
	// 	if ($id_silabus) {
	// 	$this->rows = $this->db->fetchAll("SELECT * FROM materi_silabus WHERE id_silabus = ?", $id_silabus);
	// 	$this->silabus = $this->db->fetchAll("SELECT * FROM silabus WHERE id = ?", $id_silabus);
	// }
	...
	public function editAction() {
		...
		if ($this->getRequest()->isPost()) {
			...
			if ($file_size > 134217728) {
				echo "<script>
						alert('Ukuran gambar terlalu besar!');
						window.location.href='/admin/silabus/edit/id/".$id_silabus."';
					  </script>";
				return false;
			}
			...
			if($file_audio_size > 134217728){ 
				echo "<script>
						alert('Ukuran file audio terlalu besar!');
						$this->_redirect('/admin/silabus/edit/id/".$id_silabus."');
					</script>";
				return false; 
			}
			//----------------------------------------------
...
//     $materiSilabus = $this->getMateriSilabusById($id);

//     if ($file_image && $materiSilabus->getFileImage() == $file_image) {
//         $file_path = $this->fileUploadPath . $file_image;
//         if (file_exists($file_path)) {
//             unlink($file_path);
//         }
//         $materiSilabus->setFileImage('');
//     }

//     if ($file_pdf && $materiSilabus->getFilePdf() == $file_pdf) {
//         $file_path = $this->fileUploadPath . $file_pdf;
//         if (file_exists($file_path)) {
//             unlink($file_path);
//         }
//         $materiSilabus->setFilePdf('');
//     }

//     if ($file_audio && $materiSilabus->getFileAudio() == $file_audio) {
//         $file_path = $this->fileUploadPath . $file_audio;
//         if (file_exists($file_path)) {
//             unlink($file_path);
//         }
//         $materiSilabus->setFileAudio('');
//     }

//     if ($file_video && $materiSilabus->getFileVideo() == $file_video) {
//         $file_path = $this->fileUploadPath . $file_video;
//         if (file_exists($file_path)) {
//             unlink($file_path);
//         }
//         $materiSilabus->setFileVideo('');
//     }

//     $this->saveMateriSilabus($materiSilabus);
...
// }
	...
	// 	// perbarui tabel database dengan string kosong
	// 	$params = array(
	// 		'gambar_materi_uri' => '',
	// 		'document_materi_uri' => '',
	// 		'audio_materi_uri' => '',
	...
	public function deleteAction() {
		...
		// $this->MateriSilabusService->softDeleteData($id);
		$this->MateriSilabusService->deleteData($id);
