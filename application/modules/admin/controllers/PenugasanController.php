<?php
class Admin_PenugasanController extends Zend_Controller_Action
{
	...
	{	
		if ( $this->getRequest()->isPost() )
		{
			...
			if ($file_tmp) {
				...
				$file_name = 'ketentuan/contoh_tugas-' . $title .'-'. substr(uniqid('', false), 0, 2) . '.' . $path_info['extension'];
				move_uploaded_file($file_tmp, $path . "/" . $file_name);		
			}

			$last_id = $this->PenugasanService->addTugas($id_batch, $id_peserta, $title, $file_name, $deadline_tugas);
		
			$this->_redirect('/admin/penugasan/nilai/id/'.$last_id);
		} else {
			$this->view->rows = $this->PenugasanService->getAllData();
		}
	}
	...
	{	
		...
		if ( $this->getRequest()->isPost() )
		{
			...
			$this->_redirect('/admin/penugasan/index');
		} else {
	...
	{
		$id = $this->getRequest()->getParam('id');
		$this->PenugasanService->deleteFiles($id);
		$this->_redirect('/admin/penugasan/edit/id/' . $id);
	}
