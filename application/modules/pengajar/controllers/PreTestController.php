<?php
class Pengajar_PreTestController extends Zend_Controller_Action {
	...
	public function addAction() {
		...
		if ($this->getRequest()->isPost()) {
			...
			if($file_size > 10000000){
				echo "<script>
						alert('Maks. 10MB');
						window.location.href='/pengajar/pretest/edit/id/".$id."';
					</script>";
				return false;
			}
	...
	public function deleteAction() {
		...
		$this->_redirect('/pengajar/materi-silabus-quiz/index');
	}

	public function deletefilesnontesAction() {
		$id = $this->getRequest()->getParam('id');
		$this->PreTestService->deleteFilesNontes($id);
		$this->_redirect('/pengajar/pretest/edit/id/' . $id); 
	}
}